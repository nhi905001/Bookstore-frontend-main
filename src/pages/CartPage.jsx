...
const CartPage = () => {
  ...
  const handleUpdateQuantity = async (productId, quantity) => {
    ...
    try {
      await updateCartItem(productId, quantity, userInfo.token);
      loadCart();
    } catch (error) {
  ...
  const handleRemoveItem = async (productId) => {
    try {
      await removeCartItem(productId, userInfo.token);
      loadCart();
    } catch (error) {
  ...
  if (cartItems.length === 0) {
    return (
      <div className="container mx-auto px-8 md:px-20 py-12 text-center">
        ...
        <Link
          to="/"
          className="bg-primary text-white font-bold py-3 px-8 rounded-md hover:bg-blue-600 transition-colors duration-300"
        >
          Tiếp tục mua sắm
        </Link>
      </div>
